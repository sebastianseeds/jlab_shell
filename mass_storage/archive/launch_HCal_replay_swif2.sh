...
for ((i=0; i<=$2; i++))
do
    fnameout_pattern='/farm_out/sseeds/replay_hcal_general_'$runnum'_segment'$i'.out'
    #    sbatch --output=$fnameout_pattern run_GMN_sbatch_nohodo.sh $runnum -1 0 e1209019 $i 1
    jobname='replay_hcal_general_'$runnum'_segment'$i
    ...
    script='/w/halla-scshelf2102/sbs/seeds/shell/batch_farm/run_HCal_swif2.sh'
    testfilename='/mss/halla/sbs/raw/'$eviofilename
    
    outfilename='match:e1209019_hcal_gmn_'$runnum'*seg'$i'*.root'
    logfilename='match:replay_hcal_gmn_'$runnum'*seg'$i'*.log'
